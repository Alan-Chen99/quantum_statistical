# taken from https://github.com/sundewang233/shor_algorithm_simulation/blob/0dbf9f37bd845cbd353006ded939c10d513caec2/Shor_Sequential_AQFT.py
"""
...
def get_value_a(N):
    ...
    ok = input(
        "Is the number {0} ok for a? Press 0 if not, other number if yes: ".format(a)
    )
    if ok == "0":
        if N == 3:
            print(
                "Number {0} is the only one you can use. Using {1} as value for a\n".format(
                    a, a
                )
            )
            return a
    ...
    while ok == "0":
        ...
        """ Ask user if ok """
        ok = input(
            "Is the number {0} ok for a? Press 0 if not, other number if yes: ".format(
                a
            )
        )
...
def get_factors(x_value, t_upper, N, a):
    ...
    while i >= 0:
        ...
        factors """
        print(one_factor)
        if (one_factor == 1 or one_factor == N) or (
            other_factor == 1 or other_factor == N
        ):
...
def cMULTmodN(circuit, ctl, q, aux, a, N, n, kmax):
    ...
    for i in range(0, n):
        ccphiADDmodN(
            circuit, aux, q[i], ctl, aux[n + 1], (2**i) * a % N, N, n + 1, kmax
        )
...
def get_shor_circuit(N: int, a: int):
    ...
    for i in range(0, 2 * n):
        ...
        circuit.h(up_reg)
        cMULTmodN(
            circuit, up_reg[0], down_reg, aux, a ** (2 ** (2 * n - 1 - i)), N, n, kmax
        )
